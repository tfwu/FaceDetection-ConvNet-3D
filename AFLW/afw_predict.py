...
path_dataset = "/home/yunzhu/face/AFW/testimages/"
...
def predict_afw():
    file_lst = "/home/yunzhu/face/AFW/imglst.txt"
    file_annotation = "/home/yunzhu/face/AFW/img_rects.txt"
    file_recording = "/home/yunzhu/face-eval/detections/AFW/Conv3D.txt"
    ...
    _, finetune_args, finetune_auxs = mx.model.load_checkpoint(finetune_prefix + '-01', finetune_epoch)
    ...
    rect_gt = np.zeros((num_img, MAX_FACE_NUM, 4))
    num_face_gt = np.zeros((num_img,), dtype=int)

    f = open(file_annotation, 'r').readlines()
    index_upp = len(f)
    index = 0
    index_img = 0
    ...
    while index < index_upp:
        ...
        print "Image index: %d/%d" % (index_img, num_img), f[index].strip()

        #################
        ### vgg16 rpn ###
        #################

        file_img = path_dataset + f[index].strip()
        file_name = f[index].strip()
        ...
        for i in xrange(num_face_gt[index_img]):
            index += 1
            x0, y0, x1, y1 = [float(x) for x in f[index].strip().split()]
            rect_gt[index_img][i] = np.array([x0 * resize_factor, y0 * resize_factor,
                                              x1 * resize_factor, y1 * resize_factor])

        for i in xrange(num_ellipses[index_img]):
            rect = convert_ell2rec_afw(ellipses[index_img][i], keep_ratio)
            print rect, faceness[index_img][i], [calc_iou_rectangle(rect, rect_gt[index_img][j])
                                                 for j in xrange(num_face_gt[index_img])]

        print "Ground truth rectangle:"

        for i in xrange(num_face_gt[index_img]):
            print rect_gt[index_img][i]

        for i in xrange(num_ellipses[index_img]):
            rect = convert_ell2rec_afw(ellipses[index_img][i], keep_ratio)
            f_recording.write(file_name + " " + str(faceness[index_img][i]) + " " +
                              str(rect[1] / resize_factor) + " " +
                              str(rect[0] / resize_factor) + " " +
                              str(rect[3] / resize_factor) + " " +
                              str(rect[2] / resize_factor) + "\n")
