...
def predict_fddb_fold(fold_index):
    ...
    file_recording = "/home/yunzhu/face_ext/FDDB/fold-" + fold_index + "-out.txt"
    f_recording = open(file_recording, 'w')
    ...
    _, finetune_args, finetune_auxs = mx.model.load_checkpoint(finetune_prefix + '-' + fold_index, finetune_epoch)
    ...
    # The collection of the proposed faces
    ellipses = np.zeros((num_img, num_proposal, 5))
    num_ellipses = np.zeros((num_img,), dtype=int)
    faceness = np.zeros((num_img, num_proposal))
    ...
    while index < index_upp:
        ...
        print "Image index: [" + fold_index + "] %d/%d" % (index_img, num_img), f[index].strip()
        ...
        if ground_truth.shape[0] == 0:
            f_recording.write(f[index].strip() + "\n0\n")
            index += 1
        ...
        ellipses[index_img], faceness[index_img], num_ellipses[index_img] = \
            non_maximum_suppression(tmp_ellipses, tmp_faceness, ell_output[:, 5], nms_iou, nms_ios)

        # Recording
        f_recording.write(f[index].strip() + "\n" + str(num_ellipses[index_img]) + "\n")
        ...
        for i in xrange(10):
            print ellipses[index_img][i], faceness[index_img][i],\
                [calc_iou_ellipse(ellipses[index_img][i], ellipses_gt[index_img][j])
                 for j in xrange(num_face_gt[index_img])]

        print "Ground truth ellipses:"

        for i in xrange(num_face_gt[index_img]):
            print ellipses_gt[index_img][i]

        for i in xrange(num_ellipses[index_img]):
            if metric_ellipse:
                f_recording.write(str(np.abs(ellipses[index_img][i][0] / resize_factor / 2.0)) + " " +
                                  str(np.abs(ellipses[index_img][i][1] / resize_factor / 2.0)) + " " +
                                  str(ellipses[index_img][i][2]) + " " +
                                  str(ellipses[index_img][i][4] / resize_factor) + " " +
                                  str(ellipses[index_img][i][3] / resize_factor) + " " +
                                  str(faceness[index_img][i]) + "\n")
            else:
                rect = convert_ell2rec(ellipses[index_img][i])
                f_recording.write(str(rect[1] / resize_factor) + " " +
                                  str(rect[0] / resize_factor) + " " +
                                  str((rect[3] - rect[1]) / resize_factor) + " " +
                                  str((rect[2] - rect[0]) / resize_factor) + " " +
                                  str(faceness[index_img][i]) + "\n")
    ...
    recalled = np.zeros((num_img, MAX_FACE_NUM))
    num_recalled_faces = 0
    num_faces_all_img = np.sum(num_face_gt)
    for i in xrange(num_proposal):
        for j in xrange(num_img):
            for k in xrange(num_face_gt[j]):
                if recalled[j][k] == 1:
                    continue
                if calc_iou_ellipse(ellipses_gt[j][k], ellipses[j][i]) >= metric_iou:
                    recalled[j][k] = 1
                    num_recalled_faces += 1

        print "Under %d proposal:" % i, "recalled rate: %f" % (num_recalled_faces / float(num_faces_all_img))


target_index = ["01", "02", "03", "04", "05", "06", "07", "08", "09", "10"]
# target_index = ["04", "05", "06", "07", "08", "09", "10"]
